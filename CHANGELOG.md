...
- **Gate tools (manual checkpoints and finish)**: `/boatracing setup gates [off]` gives a blue dye (right-click adds an oriented checkpoint facing your view, sneak + right-click removes the last one) and a yellow dye (right-click sets the finish gate, sneak + right-click clears it). Checkpoints and finish use different particles (`setup.gates.checkpoint-particle` = `DUST`/blue, `finish-particle` = `FLAME`) and the finish uses the same width/height as the checkpoints (only `finish-half-depth` differs). Starting AutoTrace also gives the tools, and placing the finish while recording stops the trace and generates the gates.
	- Fixed in passing: `clearfinish` / `clearpit` now actually remove the entries from the track YAML (the save routine no longer keeps null regions).
...
- **Base-managed extension system**: BoatRacing now loads extension jars from `plugins/BoatRacing/extensions/` itself instead of requiring a separate Bukkit plugin.
	...
	- Extensions can register **setup subcommands** (`ExtensionContext.registerSetupCommand`) that appear in `/boatracing setup help` and tab-completion, and store **per-track data** (`selectedTrackData`/`trackData`) inside the track YAML under `extensions.<extension>`, so add-on configuration travels with the track file and survives base saves.
	- `/boatracing setup select <track>` chooses the active track directly from setup; `/boatracing race open <track> [party|normal]` opens a race with the party flag and `race status` shows it; `RaceSessionView.partyEnabled()` exposes the flag to add-ons.
...
- **BoatRacing-PartyExtension (private paid addon, untracked `party/` project)**: party-style base-managed extension (not a Bukkit plugin) loaded from `plugins/BoatRacing/extensions/BoatRacing-PartyExtension-26.3.jar`. Gameplay is active only in races opened with party mode (`/boatracing race open <track> party`).
	- Eight abilities: Mushroom (boost), Banana (trap), Green Shell (projectile that slows on hit), Lightning (slows every rival), Super Star (invincibility + boost), Blooper (blindness ink), Coin (party points) and Bob-omb (fused explosion with knockback). Items are granted at race start, by chance on checkpoints and from item boxes.
	- Item boxes are configured per track from `/boatracing setup`: `setup select <track>`, `setup boxmarker` (marker block), `setup addbox`, `setup boxes`, `setup delbox <#|here>`, `setup clearboxes confirm`, `setup previewboxes <on|off>` and `setup boxinfo`; boxes are stored inside the track YAML (`extensions.BoatRacing-PartyExtension.boxes`) and rendered as spinning block + item displays with armor-stand and particle fallbacks.
	- Box visuals, particles and sounds only appear while configuring in setup (preview) or during a party race; ordinary races and practice ignore the add-on completely.
	- Party points from Coins and finish positions, stored by BoatRacing (`party-stats.yml` document), shown in the sidebar via the extension HUD hook; `/boatracing party help|points [player]|top|reload`.
	- Placeholders `%boatracing_party_points%`, `%boatracing_party_item%`, `%boatracing_party_top1_name%` and `%boatracing_party_top1_points%`; messages in `lang/messages_*.yml`; permissions `boatracing.party.use` (default true) and `boatracing.party.admin` (default op); box editing uses `boatracing.setup`.
...
- **Config sections**: `setup.auto-trace.*`, `setup.gates.*`, `replay.*`, `discord.*`, `cosmetics.*` (master switch, `unsupported`, density, trails/custom/disabled, effects, victory-sounds, checkpoints, titles and shop with prices/gating), `racing.victory-effects.*` and `racing.spectate-on-finish.*`.

### Changed
- **Movement checks stay cheap**: crossing detection only runs on block changes for participants of a running race, tests just the next checkpoint plus finish/pit, and uses a cached broad-phase bound (finish + pits + checkpoints + 32 blocks) to skip all math when a racer is away from the track.
...
- **Setup help and tab-completion**: the `setup` root now suggests `select`, `gates`, `autotrace`, `addalt`, `clearalt` and `setcosmetics`; `select` suggests track names; `gates` suggests `off`; `autotrace` suggests its subcommands; `addalt`/`clearalt` suggest valid checkpoint indexes; `setcosmetics` suggests `true`/`false`.
...
- README also explains the two ways to build a track (guided wizard vs manual commands) and how checkpoints, finish and pit areas work (gates are lines you cross, order enforcement, skipped checkpoints and finish/pit rules).
